...
# LES VERIFICATION POUR LE NOM D'UTILISATEUR ET LE MOT DE PASSE DANS LA BASE DE DONNEES                #
########################################################################################################
def usernameadmin_exists(username):
    # Connectez-vous à la base de données
    conn = pymysql.connect(host="localhost", user="root", password="", database="librairie")
    ...
    # Exécutez la requête SQL pour vérifier si le nom d'utilisateur existe
    sql = "SELECT COUNT(*) FROM admin WHERE username = '{}'".format(username)
    cur.execute(sql)
...
def passwordadmin_is_correct(username, password):
    # Connectez-vous à la base de données
    conn = pymysql.connect(host="localhost", user="root", password="", database="librairie")
    ...
    # Exécutez la requête SQL pour récupérer le mot de passe du nom d'utilisateur donné
    sql = "SELECT password FROM admin WHERE username = '{}'".format(username)
    cur.execute(sql)
...
def ajouter_admin(username, password):
     conn = mysql.connector.connect(user='root', password='',
                              host='localhost',
                              database='librairie')
     ...
     cursor = conn.cursor()
     cursor.execute(""" INSERT INTO admin(username, password) VALUES(%s,%s)""", reference)
     conn.commit()
     conn.close()
